...
import com.dreizak.tgv.transport.retry.GiveUpStrategy
 ...
 */
class AsyncHttpTransport private[sonatype] (client: StreamingAsyncHttpClient, handler_ : Client[HttpTransportDefinition]) extends HttpTransport {
  override val handler = handler_

  override protected def create(handler: Client[HttpTransportDefinition]): Self = new AsyncHttpTransport(client, handler)
  ...
  /**
   * Retries all hard failures except `CancelledException`s, and retries any soft failure that is
   * not a client error (4xx status code).
   */
  // FIXME
  //  override val defaultRetryOracle = new RetryOracle[HttpTransportDefinition] {
  //    def shouldRetry(response: Try[Resp]): Boolean = response match {
  //      case Failure(HttpTransportHeaderFailureException(_, httpStatus)) if httpStatus < 400 || httpStatus >= 500 => true
  //      case Failure(HttpTransportHeaderFailureException(_, _)) => false
  //      case Failure(_: CancelledException) => false
  //      case Failure(_) => true
  //      case Success(_) => false
  //    }
  //  }

  def backoffStrategy = new GiveUpStrategy()
  def retryStrategy = ??? // FIXME
}
...
object AsyncHttpTransport extends Logging {
  def apply(client: StreamingAsyncHttpClient) =
    new AsyncHttpTransport(client, new Client[HttpTransportDefinition] {
      // Any message with a non-2xx status code is a soft failure
